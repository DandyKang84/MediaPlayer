...
@Parcelize
class MusicVO(var id:String, var title:String?, var artist:String?, var albumId: String?, var duration: Int?, var likes : Int?) :
    Parcelable {
    ...
    fun getAlbumImage(context: Context, albumImageSize: Int): Bitmap? {
        ...
        if(uri != null){
            ...
            try{
                ...
                Log.d("chap17mp3_db","getAlbumImage() ${e.toString()}")
            }finally {
                try {
                    parcelFileDescriptor?.close()
                }catch (e: IOException){
                    Log.d("chap17mp3_db","getAlbumImage() parcelFileDescriptor ${e.toString()}")
                }
            }
        }
        return null
